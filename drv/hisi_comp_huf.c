...
static int check_store_huffman_block(struct bit_reader *br, __u32 bit_len)
{
	__u32 pad;
	long data;
...
static int check_fix_huffman_block(struct bit_reader *br, __u32 bit_len)
{
	long bit, len_idx, dist_code, extra;
	long code, bits;

	while (br->cur_pos < bit_len) {
...
int check_bfinal_complete_block(void *addr, __u32 bit_len)
{
	...
	while (!bfinal && br.cur_pos < bit_len) {
		...
		if (btype == DYN_TYPE) {
			...
			return check_store_huffman_block(&br, remain);
		} else if (btype == FIX_TYPE) {
			ret = check_fix_huffman_block(&br, remain);
			if (ret <= 0)
				return ret;
		}
	}
