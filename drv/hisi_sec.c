...
static int fill_cipher_bd2_mode(struct wd_cipher_msg *msg, struct hisi_sec_sqe *sqe)
{
	...
	case WD_CIPHER_ECB:
		...
		if (msg->alg == WD_CIPHER_SM4) {
			WD_ERR("Kunpeng 920 not supports ECB(SM4)!\n");
			return -WD_EINVAL;
		}
		break;
	case WD_CIPHER_CBC:
	...
	case WD_CIPHER_XTS:
		...
		break;
	default:
		WD_ERR("Invalid cipher mode type:%u!\n", msg->mode);
		return -WD_EINVAL;
	}
...
static int digest_len_check(struct wd_digest_msg *msg)
{
...
int hisi_sec_digest_send(handle_t ctx, struct wd_digest_msg *msg)
{
	...
	if (unlikely(msg->in_bytes == 0 && msg->has_next != SEC_DIGEST_FINAL)) {
