...
static int digest_length_check(struct wd_digest_msg *msg)
{
	int ret;
...
int hisi_sec_digest_send(handle_t ctx, struct wd_digest_msg *msg)
{
	...
	if (unlikely(msg->in_bytes == 0 && msg->op_state != SEC_DIGEST_FINAL)) {
		WD_ERR("kunpeng 920, digest mode not support 0 size!\n");
		return -WD_EINVAL;
	}

	ret = digest_length_check(msg);
	if (ret) {
