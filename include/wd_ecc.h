...
/**
 * wd_ecc_ctx_num() - request ctx for ecc.
 * @node:       numa node id.
 * @type:       operation type.
 * @num:        ctx number.
...
/**
 * wd_ecc_ctx_num_uninit() - UnInit ctx and schedule resources set by above init.
 */
void wd_ecc_ctx_num_uninit(void);

/**
 * wd_ecc_get_evn_param() - query the number of CTXs that meet input attributes.
 * @node:       numa node id.
 * @type:       operation type.
 * @mode:       0: sync mode, 1: async mode
 * @num:        return ctx num.
