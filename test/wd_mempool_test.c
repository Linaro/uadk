...
static void dump_mp_bp(struct wd_mempool_stats *mp_s, struct wd_blkpool_stats *bp_s)
{
	printf("---------------------------------------\n");
...
static int test_blkpool(struct test_option *opt)
{
	...
	bp = wd_blkpool_create(mp, opt->blk_size[0], opt->blk_num[0]);
	if (WD_IS_ERR(bp)) {
...
{
	struct test_opt_per_thread *opt = data;
	struct wd_blkpool_stats bp_stats = {0};
	...
	bp = wd_blkpool_create(mp, opt->blk_size, opt->blk_num);
	if (WD_IS_ERR(bp)) {
		printf("Fail to create blkpool, err %lld\n", WD_HANDLE_ERR(bp));
		return (void *)-1;
	}
...
static int test_sec_perf(struct test_option *opt)
{
	...
	for (i = 0; i < bp_thread_num; i++) {
		datas[i].bp = wd_blkpool_create(mp, opt->blk_size[i], opt->blk_num[i]);
