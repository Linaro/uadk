...
static int digest_mac_len[WD_DIGEST_TYPE_MAX] = {
...
static struct wd_digest_setting wd_digest_setting;
	...
	struct wd_digest_req *req)
{
	...
	if (sess->alg >= WD_DIGEST_TYPE_MAX || req->out_bytes == 0 ||
	    req->out_bytes > digest_mac_len[sess->alg]) {
...
{
	return wd_digest_setting.sched.poll_policy(0, 0, expt, count);
}
