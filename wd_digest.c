...
handle_t wd_digest_alloc_sess(struct wd_digest_sess_setup *setup)
{
	...
	params.ctxs = wd_digest_setting.config.ctxs;
	wd_digest_setting.sched.set_param(
...
void wd_digest_free_sess(handle_t h_sess)
{
	...
	sess->mm_ops.free(sess->mm_ops.usr, sess->key);
	if (sess->sched_key) {
		if (wd_digest_setting.sched.sched_uninit)
			wd_digest_setting.sched.sched_uninit(
				wd_digest_setting.sched.h_sched_ctx,
...
static bool wd_digest_atfork_registered;
...
int wd_digest_init(struct wd_ctx_config *config, struct wd_sched *sched)
{
	...
	if (!wd_digest_atfork_registered) {
		if (pthread_atfork(NULL, NULL, wd_digest_clear_status) == 0)
			wd_digest_atfork_registered = true;
	}
	...
	if (ret) {
		WD_ERR("failed to get driver array for digest!\n");
		goto  out_common_uninit;
	}

	wd_digest_setting.config.drv_count = drv_count;
	ret = wd_ctx_bind_drivers(&wd_digest_setting.config, NULL, WD_TYPE_V1);
	if (ret) {
	...
	if (ret) {
		WD_ERR("failed to init digest driver!\n");
...
out_free_drv_array:
	...
	wd_digest_setting.config.drv_count = 0;
out_common_uninit:
...
void wd_digest_uninit(void)
{
	...
	wd_ctx_unbind_drivers(&wd_digest_setting.config);
	wd_put_drv_array(wd_digest_setting.config.drv_array,
			     wd_digest_setting.config.drv_count);
	wd_digest_setting.config.drv_array = NULL;
	wd_digest_setting.config.drv_count = 0;
		     ...
		     struct wd_ctx_params *ctx_params)
{
	...
	ret = wd_ctx_bind_drivers(&wd_digest_setting.config,
				  wd_digest_init_attrs.ctx_config_internal,
				  WD_TYPE_V2);
	if (ret) {
		WD_ERR("failed to bind driver for digest!\n");
		goto out_common_uninit;
	}

	ret = wd_alg_init_driver(&wd_digest_setting.config);
	if (ret) {
		WD_ERR("failed to init driver for digest!\n");
		goto out_unbind_drivers;
	}
	...
	*count = 0;
	...
	do {
		...
		if (likely(req->cb))
			...
			WD_ERR("invalid: digest callback is NULL, tag %u!\n", recv_msg.tag);
