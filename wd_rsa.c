...
{
	wd_rsa_env_config.disable_env = 0;
...
int wd_rsa_ctx_num(__u32 node, __u32 type, __u32 num, __u8 mode)
{
	...
	wd_rsa_env_config.disable_env = 1;

	ret = wd_alg_table_init(&alg_table, table, table_size, num, ctx_attr);
	if (ret)
		return ret;

	return wd_alg_env_init(&wd_rsa_env_config, alg_table,
			       &wd_rsa_ops, table_size);
}
			 ...
			 __u32 *num, __u8 *is_enable)
{
	...
	*is_enable = wd_rsa_env_config.enable_internal_poll;

	return wd_alg_get_evn_param(&wd_rsa_env_config,
				    ctx_attr, num);
}
